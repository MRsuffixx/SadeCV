...
export default async function AdminFinancePage({
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <FinanceSection
        title="Transactions"
        description="Normalized Stripe and iyzico payment ledger."
      >
        <Table
          headers={[
            "Provider",
        ...
        {!data.transactions.length ? (
          <Empty>No normalized transactions have been received yet.</Empty>
        ) : null}
      </FinanceSection>
      ...
      <div className="mt-6 grid gap-6 2xl:grid-cols-2">
        <FinanceSection
          title="Subscriptions"
          description="Latest synchronized billing contracts."
        >
              ...
              <tr key={item.id}>
                ...
                <Cell>
                  {item.currentPeriodEnd
                    ? dateTime.format(item.currentPeriodEnd)
                    : "—"}
                </Cell>
              </tr>
            ))}
        ...
        <FinanceSection
          title="Donations"
          description="One-time support payments and failures."
        >
          <Table headers={["Supporter", "Provider", "Amount", "Status"]}>
            {data.donations.map((item) => (
              <tr key={item.id}>
                <Cell strong>
                  {item.user?.name ?? item.supporterEmail ?? "Guest"}
                </Cell>
                <Cell>{item.provider}</Cell>
      ...
      <FinanceSection
        ...
        description="Idempotent payment events processed by the checkout endpoint."
      >
        <Table
          headers={[
            "Provider",
        ...
        {!data.webhooks.length ? (
          <Empty>No webhook events received.</Empty>
        ) : null}
      </FinanceSection>
      ...
      <nav className="mt-6 flex items-center justify-between rounded-2xl border border-black/[0.07] bg-white p-4 text-xs font-bold">
        <Link
          ...
          className={
            data.page <= 1 ? "pointer-events-none opacity-40" : "text-[#24705d]"
          }
  ...
  return (
    <article className="rounded-2xl border border-black/[0.07] bg-white p-4">
      <div className="flex items-center gap-3">
        <span className="grid size-9 place-items-center rounded-xl bg-[#e7f0eb] text-[#2b725f]">
          <Icon size={16} />
        </span>
        <div>
          <p className="text-xl font-black">{value}</p>
          <p className="text-[10px] font-bold text-[#7d8682]">{label} total</p>
        </div>
      </div>
    </article>
  );
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-[1.5rem] border border-black/[0.07] bg-white">
      <div className="border-b border-black/[0.06] p-5">
        ...
        <p className="mt-1 text-xs text-[#7b8580]">{description}</p>
      </div>
      {children}
    </section>
  );
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-xs">
        ...
        <tbody className="divide-y divide-black/[0.055]">{children}</tbody>
      </table>
    </div>
  );
}
...
function Cell({
  ...
  return (
    <td
      className={`max-w-[280px] truncate px-5 py-3.5 whitespace-nowrap ${strong ? "font-extrabold" : "text-[#67716c]"} ${mono ? "font-mono text-[10px]" : ""}`}
    >
      {children}
    </td>
  );
...
function Status({ value }: { value: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-1 text-[9px] font-black ${good ? "bg-[#def0e7] text-[#216652]" : "bg-[#f7e4e0] text-[#98483b]"}`}
    >
      {value}
    </span>
  );
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <p className="p-8 text-center text-xs font-bold text-[#858e8a]">
      {children}
    </p>
  );
}
