...
export function AtsDomTemplate({ presentation, definition }: DomTemplateProps) {
  ...
  return (
    ...
    >
      <header
        className="border-b-2 pb-[3.2%]"
        style={{ borderColor: presentation.theme.accentColor }}
      >
        <h1
          className={`${fonts.heading} text-[clamp(1.3rem,3.7vw,2.65rem)] leading-none font-black tracking-[-0.04em]`}
        >
          {presentation.identity.name}
        </h1>
        <p
          className="mt-[1.6%] text-[clamp(0.48rem,1vw,0.72rem)] font-black tracking-[0.14em] uppercase"
