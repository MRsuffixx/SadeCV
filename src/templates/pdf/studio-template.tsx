...
const styles = StyleSheet.create({
  ...
  name: { marginTop: 24, fontSize: 23, lineHeight: 0.96, letterSpacing: -0.7, color: "#ffffff" },
...
export function StudioPdfTemplate({ presentation }: { presentation: ResumePresentation }) {
    ...
    <Document title={presentation.identity.name} subject="Creative professional resume">
      <Page size="A4" wrap style={[styles.page, { fontFamily: theme.body }]}>
        ...
        <View style={styles.sidebar}>
          ...
          <Text style={[styles.connectLabel, { fontFamily: theme.heading }]}>Connect</Text>
          <PdfContactList items={[...presentation.contact, ...presentation.links]} presentation={presentation} inverse />
          <View style={styles.sidebarSections}>{sidebar.map((section) => <PdfSection key={section.id} section={section} presentation={presentation} inverse />)}</View>
        </View>
        <View style={styles.main}>
          <View style={[styles.mainRule, { backgroundColor: presentation.theme.accentColor }]} />
          <Text style={[styles.mainLabel, { fontFamily: theme.heading }]}>Selected work & experience</Text>
          {main.map((section) => <PdfSection key={section.id} section={section} presentation={presentation} />)}
        </View>
        <PdfPageNumber />
      </Page>
    </Document>
